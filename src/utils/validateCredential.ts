// Define types for the input parameters (vcSpec and vcClaims)
interface VcSpec {
...
interface VcClaims {
  type: string[];
  credentialSubject: Record<string, Record<string, string | number>>;
}

export default function validateCredential(vcSpec: VcSpec, vcClaims: VcClaims): boolean {
  const { credentialType } = vcSpec;
